...
# Includes scaling to deal with potential underflow problems
import numpy as np
    ...
    def fit(self, X, max_iter=30, seed = 123):
        ...
        for it in range(max_iter):
            ...
            for n in range(N):
                ...
                scale[0] = alpha[0].sum()
                alpha[0] /= scale[0] # alpha prime transformed to alpha hat
            ...
            for n in range(N):
                ...
                for i in range(self.M):
                    for j in range(self.M):
                        for t in range(T-1):
                            a_num[i,j] += alphas[n][t,i] * self.A[i,j] * self.B[j, x[t+1]] * betas[n][t+1,j] /scales[n][t+1]
