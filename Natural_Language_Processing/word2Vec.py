# A manual implementation of word2Vec using numpy
import json
import numpy as np
...
def get_wiki():
    ...
    # Dictionary that stores count for each word in the vocabulary
    all_word_counts = {}
    for f in files:  # Loop files 
        for line in open(f, encoding='utf8'):  # Open individual file
            if line and line[0] not in '[*-|=\{\}':  # Remove punctuation
                s = remove_punctuation(line).lower().split()
                if len(s) > 1: 
                    for word in s:  # Get each word in the sentence
                        if word not in all_word_counts:
                            all_word_counts[word] = 0
                        all_word_counts[word] += 1
    print("Finished counting")

    # Limit the vocab size to 20000
    V = min(V, len(all_word_counts))
    all_word_counts = sorted(all_word_counts.items(), key=lambda x: x[1], reverse=True)  # Sort in descending order


    # Replace uncommon words with <UNK> (unknown)
...
def train_model(savedir):
    ...
    for epoch in range(epochs):
        ...
        randomly_order_positions = np.random.choice(
            len(sentence),
            size=len(sentence),
            replace=False,
        )

        for pos in randomly_order_positions:
            # The middle word
            word = sentence[pos]
            ...
            c = sgd(word, targets, 1.0, learning_rate, W, V)
            cost += c
...
def get_negative_sampling_distribution(sentences, vocab_size):
    # p(n) = prob of word occurring
    # We would like to sample the negative samples such that words occur more often should be sampled more often
...
def get_context(pos, sentence, window_size):
    # Input: a sentence of the form x x x c c c pos c c c x x x 
    # Output: the context word indices: c c c c c
    ...
    for ctx_pos, ctx_word_idx in enumerate(sentence[start:end_], start=start):
        if ctx_pos != pos:
            # Don't inlcude the input word itself as a target
...
def sgd(input_, targets, label, learning_rate, W, V):
    ...
    # Return the cost (binary cross entropy)
...
def analogy(pos1, neg1, pos2, neg2, word2idx, idx2word, W):
    ...
    print("Testing: %s - %s = %s" % (pos1, neg1, pos2, neg2))
