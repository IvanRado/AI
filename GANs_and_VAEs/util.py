...
def get_celeb(limit=None):
  ...
  if not os.path.exists('../LargeFiles/img_align_celeba-cropped'):

    # check for original data
    if not os.path.exists('../LargeFiles/img_align_celeba'):
      # download the file and place it here
      if not os.path.exists('../LargeFiles/img_align_celeba.zip'):
        print("Downloading img_align_celeba.zip...")
        download_file(
          '0B7EVK8r0v71pZjFTYXZWM3FlRnM',
          '../LargeFiles/img_align_celeba.zip'
        )

      # unzip the file
      print("Extracting img_align_celeba.zip...")
      with zipfile.ZipFile('../LargeFiles/img_align_celeba.zip') as zf:
        zip_dir = zf.namelist()[0]
        zf.extractall('../LargeFiles')


    # load in the original images
    filenames = glob("../LargeFiles/img_align_celeba/*.jpg")
    N = len(filenames)
    print("Found %d files!" % N)


    # crop the images to 64x64
    os.mkdir('../LargeFiles/img_align_celeba-cropped')
    print("Cropping images, please wait...")

    for i in range(N):
      crop_and_resave(filenames[i], '../LargeFiles/img_align_celeba-cropped')
      if i % 1000 == 0:
        print("%d/%d" % (i, N))


  # make sure to return the cropped version
  filenames = glob("../LargeFiles/img_align_celeba-cropped/*.jpg")
  return filenames
